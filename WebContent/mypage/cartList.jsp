			...
			<c:otherwise>
				<table id="cartList">
					...
					<c:forEach var="cart" items="${cartList}" varStatus="status">
						<tr>
							<td>
								<a href="shop.do?command=productDetail&pseq=${cart.pseq}">
									<h3>${cart.pname}</h3>
								</a>
							</td>
							<td>${cart.quantity}</td>
							...
							<td>
								<fmt:formatDate value="${cart.indate}" type="date" />
							</td>
							<td>
								<input type="checkbox" name="cseq" value="${cart.cseq}" id="cseq_${cart.cseq}" />
							</td>
						</tr>
					</c:forEach>
					<tr>
